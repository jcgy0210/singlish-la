...
interface Prop {
  quizName: string;
  questionsAndOptions: { id:number; question: string; options: string[]; correctAnswer: string}[];
  onClose: () => void;
}

const Quiz = ({ quizName, questionsAndOptions, onClose }: Prop) => {
  ...
  const score = questionsAndOptions.reduce((acc, question) => {
        ...
        {!showResults ? (
          ...
          <div className="text-center">
            <h2 className="text-2xl font-semibold mb-4">
              Your Score: {score} / {questionsAndOptions.length}
            </h2>
            <button
